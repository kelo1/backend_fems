...
/* Route::get('/csrf-test', function (Request $request) {
  return response()->json(['csrf_token' => csrf_token(), 'session_id' => session()->getId()]);
...
Route::post('/user/login', [AuthController::class, 'signIn'])->middleware(['web']);


//FEMS Admin Routes
//Route::post('/fems_admin/login', [FEMSAdminController::class, 'login'])->name('login_fems_admin');
Route::post('/fems_admin/login', [FEMSAdminController::class, 'login'])->middleware(['web'])->name('login_fems_admin');
...
Route::group(['middleware'=>['web','auth:sanctum']], function(){
  ...
  Route::post('/user/user_by_id/{id}', [AuthController::class, 'getUserByID'])->name('user_by_id'); //////new
  ...
  Route::get('/billing/fsa/{fsaid}', [BillingController::class, 'billingByFSA'])->name('billing_by_fsa_agent');
  ...
  Route::get('/fsa/invoice/all/{fsaId}', [InvoicesbyFSAController::class, 'getInvoiceByFSA'])->name('fsa_invoicing_by_FSA');
  ...
  Route::delete('/service_provider_device/{id}', [ServiceProviderDevicesController::class, 'destroy'])->name('service_provider_devices_delete');
